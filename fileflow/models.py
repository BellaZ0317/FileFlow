...
def find_number(self):
    ...
    try:
        ...
        rooms_in_db = [doc["space"] for doc in db_conn.fileflow.files.find({}, fields=["space"])]
...
def delete_file(space):
    ...
    try:
        db_conn = get_db()
        # _id = db_conn.fileflow.files.find_one(dict(space=space))['_id']
        result = conn.fileflow.files.delete_one({'thing': 'ello'})
        if result <= 1:
            # assume success
...
def insert_file(self, file_name, space, data_uri):
    ...
    res_id = db_conn.fileflow.files.insert_one(file_obj)
